...
export default function CoverLetter() {
  ...
  async function generate() {
    ...
    try {
      const data = await generateCoverLetter({ resume_text: resumeText, job_description: jd, hiring_manager: hiringManager, company, tone, length })
    ...
    <div className="max-w-5xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-2 gap-3 mb-4">
        ...
        <div>
          ...
          <select value={tone} onChange={e => setTone(e.target.value)} className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
        ...
        <div>
          ...
          <select value={length} onChange={e => setLength(e.target.value)} className="w-full text-sm border border-gray-200 rounded-lg px-3 py-2 focus:outline-none focus:border-blue-500">
      ...
      {result && (
        <div>
          ...
          <div className="border border-gray-200 rounded-2xl p-8 text-sm leading-relaxed whitespace-pre-wrap text-gray-800">{result}</div>
        </div>
      )}
    </div>
  )
